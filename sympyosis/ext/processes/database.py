...
class ProcessModel:
    def __init__(
        self, service: str, pid: int, status: ProcessStatus | int, *, db: Database
    ):
    ...
    def __repr__(self):
        return f"{type(self).__name__}(service={self.service!r}, pid={self.pid}, status={self.status})"
...
class Database(AutoInject):
    ...
    def add_process(self, service: str, pid: int, status: ProcessStatus):
        self.db.execute(
            "INSERT INTO Processes(service, pid, status) VALUES(?, ?, ?)",
            (service, pid, status.value),
        )
        self.db.commit()
